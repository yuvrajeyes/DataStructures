...
int update_internal(int pos, int add, int l, int r, int id, Node* ST, int& blen) {
...
int query(int L, int R, int K, int l = 1, int r = n + 1, int x = 1, int lvl = 0) {
    if (r <= L || R <= l) return 0;
    if (l >= L && r <= R) 
    ...
    int lc = (x << 1);
    int rc = (x << 1) | 1;
    return query(L, R, K, l, mid, lc, lvl + 1) + query(L, R, K, mid, r, rc, lvl + 1);
}

void update(int pos, int val, int l = 1, int r = n + 1, int x = 1, int lvl = 0) {
